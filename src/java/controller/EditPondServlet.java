 ...
 * @author Alif Danish
 */
@WebServlet(name = "EditPondServlet", urlPatterns = { "/EditPondServlet" })
@MultipartConfig( maxFileSize = 16177216 )
public class EditPondServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        EditPondBean editpondBean = new EditPondBean();
        
        editpondBean.setPondname(pondname);
        editpondBean.setPondDesc(description);
        editpondBean.setFee(fee);
        ...
        if(image != null){
            ...
            editpondBean.setImage((Part) image);

        }
        
        EditPondDao editpondDao = new EditPondDao();
        String pondRegistered = "null";
        
        try{
            pondRegistered = editpondDao.editPond(editpondBean);
        }catch(ClassNotFoundException ex){
            Logger.getLogger(EditEventServlet.class.getName()).log(Level.SEVERE,null,ex);
        }
        if(pondRegistered.equals("SUCCESS")){
            request.setAttribute("errMessage", "You have successfully added a new fishing pond");
